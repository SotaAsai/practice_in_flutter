...
class _PieChartScreen extends State<PieChartScreen> {
  ...
  Widget build(BuildContext context) {
    return Scaffold(
      appBar: AppBar(title: const Text("パイチャート")),
      body: SizedBox(
...
class RectPainter extends CustomPainter {
  ...
  void paint(Canvas canvas, Size size) {
    ...
    canvas.drawArc(rect, pi * (-90 / 180), (pi * 2) * (100 / 360), true,
        paint1); // -90π/180から10π/180まで

    Paint paint2 = Paint()..color = Colors.green;

    canvas.drawArc(rect, pi * ((100 - 90) / 180), (pi * 2) * (140 / 360), true,
        paint2); // -10π/180から

    Paint paint3 = Paint()..color = Colors.blue;
    canvas.drawArc(
        rect, (pi * (240 - 90) / 180), (pi * 2) * (120 / 360), true, paint3);

    Paint paint4 = Paint()..color = Colors.white;
