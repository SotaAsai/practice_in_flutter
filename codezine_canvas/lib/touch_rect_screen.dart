...
class _TouchRectScreen extends State<TouchRectScreen> {
  ...
  Widget build(BuildContext context) {
    return Scaffold(
        appBar: AppBar(title: const Text("タッチスクリーン")),
...
class TapRectController {
  ...
  Future<ui.Image> onChopRequest() async {
    double minScale =
        min(screen!.width / image!.width, screen!.height / image!.height);
    double upScale = 1 / minScale;

    var pictureRecorder = ui.PictureRecorder();
    ui.Canvas canvas = ui.Canvas(pictureRecorder);

    Rect chopSrc = Rect.fromLTRB(start!.dx * upScale, start!.dy * upScale,
        end!.dx * upScale, end!.dy * upScale);
    Rect chopDst = Rect.fromLTRB(
        0, 0, (end!.dx - start!.dx) * upScale, (end!.dy - start!.dy) * upScale);

    canvas.drawImageRect(image!, chopSrc, chopDst, Paint());
...
class BackgroundPainter extends CustomPainter {
  ...
  void paint(ui.Canvas canvas, ui.Size size) {
    double minScale =
        min(size.width / _image.width, size.height / _image.height);
...
class TouchRectPainter extends CustomPainter {
  ...
  void paint(Canvas canvas, Size size) {
    ...
    Paint paint1 = Paint()..color = const Color.fromRGBO(255, 255, 255, 0.5);
