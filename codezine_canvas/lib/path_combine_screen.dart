...
class _PathCombineScreen extends State<PathCombineScreen> {
  ...
  Widget build(BuildContext context) {
    return Scaffold(
      appBar: AppBar(title: const Text("パスの合成")),
      body: SizedBox(
...
class RectPainter extends CustomPainter {
  ...
  void paint(Canvas canvas, Size size) {
    ...
    canvas.scale(1, -1);
    canvas.drawPath(
        Path.combine(PathOperation.difference, path1, path2), paint);
    ...
    canvas.scale(1, -1);
    canvas.drawPath(
        Path.combine(PathOperation.reverseDifference, path1, path2), paint);
