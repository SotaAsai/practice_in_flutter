...
class _Sample1Screen extends State<Sample1Screen> {
  ...
  Widget build(BuildContext context) {
    return Scaffold(
      appBar: AppBar(title: const Text("サンプル(1)")),
      body: SizedBox(
...
class SamplePainter extends CustomPainter {
  ...
  void paint(Canvas canvas, Size size) {
    ...
    //  マージン10での画面一杯の四角
    Rect rect1 =
        const Offset(20.0, 20.0) & Size(size.width - 40.0, size.height - 40.0);
    ...
    Rect rect2 = Rect.fromLTWH(
        size.width / 2 - 50.0, size.height / 2 - 50.0, 100.0, 100.0);
    canvas.drawRect(rect2, paint2);

    Paint paint3 = Paint()..color = Colors.black;
